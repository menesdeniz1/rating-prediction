...
float predict(int target_user, int target_movie) {
    ...
    std::vector<std::pair<int, float>>::iterator end = similiarUsers_and_similarities_to_target.end();
...
int main() {
    ...
    outFile.open("C:\\Users\\MSI-GL65\\Desktop\\output.csv", std::ios::out);

    readTrain("C:\\Users\\MSI-GL65\\Desktop\\train.csv");
    readTest("C:\\Users\\MSI-GL65\\Desktop\\test.csv");
    ...
    for (auto targetUser_itr = targetUser_id.begin(); targetUser_itr != targetUser_id.end(); targetUser_itr++) {
        ...
        float pre = predict(valueFirst, valueSecond);
        outFile << i++ << "," << pre << endl;
